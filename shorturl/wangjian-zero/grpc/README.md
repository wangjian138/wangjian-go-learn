...
[![GoDoc](https://godoc.org/shorturl/wangjian-zero/grpc?status.svg)](https://godoc.org/shorturl/wangjian-zero/grpc)
...
```
$ go get -u shorturl/wangjian-zero/grpc
```

With Go module support (Go 1.11+), simply `import "shorturl/wangjian-zero/grpc"` in
...
Documentation
-------------
- See [godoc](https://godoc.org/shorturl/wangjian-zero/grpc) for package and API
  descriptions.
...
```
$ go get -u shorturl/wangjian-zero/grpc
package shorturl/wangjian-zero/grpc: unrecognized import path "shorturl/wangjian-zero/grpc" (https fetch: Get https://shorturl/wangjian-zero/grpc?go-get=1: dial tcp 216.239.37.1:443: i/o timeout)
```
...
- Without Go module support: `git clone` the repo manually:
  ...
  ```
  git clone https://github.com/grpc/grpc-go.git $GOPATH/src/shorturl/wangjian-zero/grpc
  ```
...
- With Go module support: it is possible to use the `replace` feature of `go
  ...
  ```
  go mod edit -replace=shorturl/wangjian-zero/grpc=github.com/grpc/grpc-go@latest
  go mod tidy
  go mod vendor
  go build -mod=vendor
  ```
...
require (
    shorturl/wangjian-zero/grpc v1.27.0
)
```
...
possible reasons, including:
 ...
 1. Keepalive parameters caused connection shutdown, for example if you have configured
    ...
    If this is the case, you may want to increase your [MaxConnectionAgeGrace](https://pkg.go.dev/shorturl/wangjian-zero/grpc/keepalive?tab=doc#ServerParameters),
