...
servers is to use
[`encoding.RegisterCompressor`](https://godoc.org/shorturl/wangjian-zero/grpc/encoding#RegisterCompressor)
...
it via the
[`UseCompressor`](https://godoc.org/shorturl/wangjian-zero/grpc#UseCompressor)
`CallOption`.  Remember that `CallOption`s may be turned into defaults for all
calls from a `ClientConn` by using the
[`WithDefaultCallOptions`](https://godoc.org/shorturl/wangjian-zero/grpc#WithDefaultCallOptions)
