...
create grpc
[`credentials.TransportCredentials`](https://godoc.org/shorturl/wangjian-zero/grpc/credentials#TransportCredentials)
base on TLS. Refer to the
[godoc](https://godoc.org/shorturl/wangjian-zero/grpc/credentials) for details.
...
[`credentials.NewServerTLSFromFile`](https://godoc.org/shorturl/wangjian-zero/grpc/credentials#NewServerTLSFromFile).

On client side, we provide the path to the "ca.pem" to configure TLS and create
the client credential using
[`credentials.NewClientTLSFromFile`](https://godoc.org/shorturl/wangjian-zero/grpc/credentials#NewClientTLSFromFile).
...
[`grpc.Creds`](https://godoc.org/shorturl/wangjian-zero/grpc#Creds)) and let client dial
to the server with the created client credential (by calling
[`grpc.WithTransportCredentials`](https://godoc.org/shorturl/wangjian-zero/grpc#WithTransportCredentials))
...
[`alts.DefaultServerOptions`](https://godoc.org/shorturl/wangjian-zero/grpc/credentials/alts#DefaultServerOptions)
to get the configuration for alts and then provide the configuration to
[`alts.NewServerCreds`](https://godoc.org/shorturl/wangjian-zero/grpc/credentials/alts#NewServerCreds)
to create the server credential based upon alts.

On client side, first call
[`alts.DefaultClientOptions`](https://godoc.org/shorturl/wangjian-zero/grpc/credentials/alts#DefaultClientOptions)
to get the configuration for alts and then provide the configuration to
[`alts.NewClientCreds`](https://godoc.org/shorturl/wangjian-zero/grpc/credentials/alts#NewClientCreds)
